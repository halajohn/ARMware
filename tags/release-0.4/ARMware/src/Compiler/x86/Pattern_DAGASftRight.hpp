...
#ifndef Pattern_DAGASftRight_hpp
#define Pattern_DAGASftRight_hpp
...
namespace ARMware
{
  namespace Pattern_DAGASftRight
  {
    inline LIR *
                         ...
                         Variable * const var3)
    {
      ...
      //
      // The shift register value must be >= 1 & <= 31.
      // Thus I don't need to check the value of the register anymore.
      //
      // See Inst/InstDPI_DP.tcc: gen_shifter_operand().
      Variable * const tmp = gp_chunk->get_new_temp();
      
      curr_bb->append_lir<true>(new LIR(static_cast<LIRMov *>(0), var2, tmp));
      
      tmp->set_preallocated_reg(new Register(Register::ECX, true));
      
      // :NOTE: Wei 2005-Mar-09:
      //
      // Because the flags affected by ARM & Intel are the same,
      // thus I can use the flags directly.
      return curr_bb->append_lir<true>(new LIR(static_cast<LIRASftRight *>(0), var1, tmp, var3));
    }
    
    inline void
    emit_lir_code(BasicBlock * const curr_bb, DAGNode * const node)
    {
      assert(DAG_ASFT_RIGHT == node->kind());
      ...
      else
      {
        ...
        case DAG_VAR_CONST:
          ...
          {
          case DAG_VAR_CONST:
            assert(!"The 1st operand of the ASFT_RIGHT instruction in x86 can't be a constant value.");
            main_lir = 0;
            break;
        ...
        case DAG_VAR_GLOBAL:
          ...
          case DAG_VAR_CONST:
            // :NOTE: Wei 2005-Mar-09:
            //
            // The shift immediate value must be <= 31,
            // see Inst/InstDPI_DP.tcc: gen_shifter_operand().
            assert(child_1.peek_first_usable_attached_var()->const_num() <= 31);
            
            main_lir = curr_bb->append_lir<true>(new LIR(static_cast<LIRASftRight *>(0),
