...
#ifndef Pattern_DAGEor_hpp
#define Pattern_DAGEor_hpp

#if ENABLE_DYNAMIC_TRANSLATOR

namespace ARMware
{
  namespace Pattern_DAGEor
  {
    inline void
    emit_lir_code(BasicBlock * const curr_bb, DAGNode * const node)
    {
      assert(DAG_EOR == node->kind());
      ...
      else
      {
        ...
        case DAG_VAR_CONST:
          ...
          {
          case DAG_VAR_CONST:
            assert((node->in_flags() | node->out_flags()) != 0);
            
            {
              ...
              main_lir = curr_bb->append_lir<true>(new LIR(static_cast<LIREor *>(0),
                                                           tmp,
