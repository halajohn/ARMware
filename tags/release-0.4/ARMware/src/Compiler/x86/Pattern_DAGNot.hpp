...
#ifndef Pattern_DAGNot_hpp
#define Pattern_DAGNot_hpp

#if ENABLE_DYNAMIC_TRANSLATOR

namespace ARMware
{
  namespace Pattern_DAGNot
  {
    inline void
    emit_lir_code(BasicBlock * const curr_bb, DAGNode * const node)
    {
      assert(DAG_NOT == node->kind());
      ...
      else
      {
        ...
        {
        case DAG_VAR_CONST:
          assert(!"Should not reach here, Because NOT a constant should be eliminated by constant folding.");
          break;
          
        case DAG_VAR_TEMP:
        case DAG_VAR_GLOBAL:
          curr_bb->append_lir<true>(new LIR(static_cast<LIRNot *>(0),
