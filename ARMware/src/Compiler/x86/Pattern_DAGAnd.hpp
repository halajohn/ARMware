...
#ifndef Pattern_DAGAnd_hpp
#define Pattern_DAGAnd_hpp

#if ENABLE_DYNAMIC_TRANSLATOR

namespace ARMware
{
  namespace Pattern_DAGAnd
  {
    inline void
    emit_lir_code(BasicBlock * const curr_bb, DAGNode * const node)
    {
      assert(DAG_AND == node->kind());
      
      // :NOTE: Wei 2005-Mar-09:
      //
      // The out flags of the AND instruction should _not_ include CARRY & OVERFLOW.
      ...
      else
      {
        ...
        case DAG_VAR_CONST:
          ...
          {
          case DAG_VAR_CONST:
            assert(!"And of 2 constants. This shouldn't be happened, because I have already implement AND constant folding.");
            main_lir = 0;
            break;
          ...
          case DAG_VAR_GLOBAL:
            ...
            main_lir = curr_bb->append_lir<true>(new LIR(static_cast<LIRAnd *>(0),
