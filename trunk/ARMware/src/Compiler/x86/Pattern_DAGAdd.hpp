...
#ifndef Pattern_DAGAdd_hpp
#define Pattern_DAGAdd_hpp

#if ENABLE_DYNAMIC_TRANSLATOR

namespace ARMware
{
  namespace Pattern_DAGAdd
  {
    inline void
    emit_lir_code(BasicBlock * const curr_bb, DAGNode * const node)
    {
      assert(DAG_ADD == node->kind());
      ...
      else
      {
        ...
        case DAG_VAR_CONST:
          ...
          {
          case DAG_VAR_CONST:
            assert(!"Addition of 2 constants. This shouldn't be happened, because I have already implement ADD constant folding.");
            main_lir = 0;
            break;
          ...
          case DAG_VAR_GLOBAL:
            ...
            //
            // Because the flags affected by ARM ADD & Intel ADD are the same,
            // thus I can use the flags directly.
            main_lir = curr_bb->append_lir<true>(new LIR(static_cast<LIRAdd *>(0),
